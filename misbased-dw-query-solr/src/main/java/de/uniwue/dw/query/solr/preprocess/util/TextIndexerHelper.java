...
import static de.uniwue.dw.query.solr.preprocess.util.NegationType.*;

public class TextIndexerHelper {
  private static int replacementCounter;

  private  Map<String, String> replacementMap;

  private static HashMap<String, String> filename2content = new HashMap<String, String>();

  private static ArrayList<String> conjunctionTerms = null;
  ...
  // public static void uploadDocument(long id, String text) throws SolrServerException, IOException
  // {
  ...
  // }
  //
  // public static void uploadDocument(long id, String text, SolrServer server)
  ...
  // public static List<List<String>> getNegInText(String text) {
  // if (text == null)
  // text = "";
  // Pattern preNegTermPattern = Pattern.compile(getPreNegTermPattern(), Pattern.CASE_INSENSITIVE);
  // Pattern postNegTermPattern = Pattern.compile(getPostNegTermPattern(),
  ...
  public  AnalysedText getNegationsInText(String text) {
    if (text == null)
      text = "";
    Pattern preNegTermPattern = Pattern.compile(getPreNegTermPattern(), Pattern.CASE_INSENSITIVE);
  ...
  public static AnalysedText getNegInText(ArrayList<String> segments, Pattern preNegTermPattern,
          Pattern postNegTermPattern, Pattern pseudoNegTermPattern) {
    AnalysedText text = new AnalysedText();
    Iterator<String> it = segments.iterator();
    int position = 1;
    while (it.hasNext()) {
      String curSegment = it.next();
      if (!curSegment.isEmpty()) {
        if (checkNeg(curSegment, preNegTermPattern, postNegTermPattern, pseudoNegTermPattern)) {
          text.add(curSegment, position, NEGATED);
        } else {
          text.add(curSegment, position, NOT_NEGATED);
        }
        position += curSegment.length() + 1;
      }
    }
    return text;
  }
  ...
  private  ArrayList<String> getSegments(String processedText,
          ArrayList<String> conjunctionTerms) {
    String resultString = "";
  ...
  public static String getWordsFromFile(String filename, boolean canBePrefix) {
    String content = filename2content.get(filename);
    if (content == null) {
      content = readWordsFromFile(filename, canBePrefix);
      filename2content.put(filename, content);
    }
    return content;
  }

  public static String readWordsFromFile(String filename, boolean canBePrefix) {
    String result = "";
    // try {
  ...
  public static ArrayList<String> getConjunctionTerms() {
    if (conjunctionTerms == null)
      conjunctionTerms = readConjunctionTerms();
    return conjunctionTerms;
  }

  public static ArrayList<String> readConjunctionTerms() {
  ...
  public static Scanner getScanner(String filename) {
    InputStream stream = TextIndexerHelper.class.getClassLoader().getResourceAsStream(filename);
